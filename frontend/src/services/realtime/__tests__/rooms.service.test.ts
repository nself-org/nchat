...
describe("RoomsService", () => {
  ...
  describe.skip("room management", () => {
    ...
    it("should leave a room", async () => {
      ...
      expect(mockRealtimeClient.emit).toHaveBeenCalledWith("room:leave", {
        roomName: "general",
      });
    });
    ...
    it("should throw when not connected", async () => {
      mockRealtimeClient._isConnected = false;
      await expect(service.joinRoom("general")).rejects.toThrow(
        "Not connected",
      );
    });
  });
  ...
  describe.skip("messaging", () => {
    ...
    it("should send a message with options", async () => {
      await service.joinRoom("general");
      (mockRealtimeClient.emitAsync as jest.Mock).mockResolvedValueOnce({
        messageId: "msg-124",
      });
    ...
    it("should throw when sending to unjoined room", async () => {
      await expect(service.sendMessage("unknown", "Hello!")).rejects.toThrow(
        "Not in room",
      );
    });
    ...
    it("should throw when not connected", async () => {
      mockRealtimeClient._isConnected = false;
      await expect(service.sendMessage("general", "Hello!")).rejects.toThrow(
        "Not connected",
      );
    });
  });

...
export class MessageExportService {
  ...
  async exportMessages(
    ...
    try {
      ...
      logger.error(
        "MessageExportService.exportMessages failed",
        error as Error,
      );
      return {
        success: false,
    ...
    try {
      ...
      if (messages.length === 0) {
        ...
        job.result = {
          ...
          downloadUrl: "",
          expiresAt: new Date(
            Date.now() + EXPORT_EXPIRY_HOURS * 60 * 60 * 1000,
          ),
  ...
  private isSystemMessage(message: ExtendedMessage): boolean {
    ...
    return (
      systemTypes.includes(message.type) ||
      systemTypes.includes(message.extendedType || "")
    );
  }
  ...
  private exportToMarkdown(
    ...
    for (const msg of messages) {
      ...
      lines.push(
        `**${msg.author.displayName}** - ${format(new Date(msg.createdAt), "MMM d, yyyy h:mm a")}${msg.isEdited ? " (edited)" : ""}`,
      );
      lines.push("");
      ...
      if (msg.poll) {
        ...
        for (const opt of msg.poll.options) {
          const bar =
            "█".repeat(Math.round(opt.percentage / 5)) +
            "░".repeat(20 - Math.round(opt.percentage / 5));
      ...
      if (msg.location) {
        lines.push(
          `📍 **Location:** ${msg.location.name || ""} ${msg.location.address || ""}`,
        );
        lines.push(

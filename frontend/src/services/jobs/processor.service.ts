...
async function processEmailDigest(
  job: Job<JobData>,
): Promise<JobResult<EmailDigestResult>> {
...
async function processCleanupExpired(
  job: Job<JobData>,
): Promise<JobResult<CleanupResult>> {
  ...
  try {
    ...
    const hours = payload.olderThanDays
      ? payload.olderThanDays * 24
      : payload.olderThanHours || 24;
 ...
 */
async function processIndexSearch(
  job: Job<JobData>,
): Promise<JobResult<IndexSearchResult>> {
      ...
      case "update":
        ...
        for (const entityId of payload.entityIds) {
          ...
          await job.updateProgress(
            10 +
              Math.floor(
                ((indexedCount + failedCount) / payload.entityIds.length) * 80,
              ),
          );
        }
        break;
...
async function processFile(
  job: Job<JobData>,
): Promise<JobResult<ProcessFileResult>> {
...
async function processSendNotification(
  ...
  try {
    ...
    for (const userId of payload.userIds) {
      ...
      await job.updateProgress(
        10 +
          Math.floor(((sentCount + failedCount) / payload.userIds.length) * 80),
      );
    }
...
async function processSendEmail(
  ...
  JobResult<{ messageId: string; accepted: string[]; rejected: string[] }>
> {
 ...
 * Process custom job
 */
async function processCustomJob(
  job: Job<JobData>,
 ...
 */
const processorRegistry: Record<
  NchatJobType,
  (job: Job<JobData>) => Promise<JobResult<unknown>>
> = {
...
export class ProcessorService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      log.info("Initializing processor service", {
      ...
      this.isInitialized = true;
      log.info("Processor service initialized", {
  ...
  getWorkerStatus(
    queueName: QueueName,
  ): { running: boolean; paused: boolean } | null {
    const worker = this.workers.get(queueName);
 ...
 */
export function getProcessorService(

...
export class AIModerationService {
  ...
  async scanText(
    ...
    try {
      ...
      if (scores.profanity >= this.config.thresholds.profanity)
  ...
  private async checkSpam(
    ...
    if (
      emojiCount > 10 ||
      (content.length > 0 && emojiCount / content.length > 0.3)
    ) {
  ...
  async scanBatch(
    ...
    return Promise.all(
      items.map((item) =>
        item.type === "text"
          ? this.scanText(item.content)
          : this.scanImage(item.content),
      ),
    );
  }

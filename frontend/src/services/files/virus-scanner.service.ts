...
export class VirusScannerService {
  ...
  async scanFile(
    ...
    // Try primary backend
    let result = await this.scanWithBackend(
      buffer,
      this.config.backend,
      options,
    );
    ...
    if (
      ...
      this.config.fallbackBackend !== "none"
    ) {
      ...
      result = await this.scanWithBackend(
        buffer,
        this.config.fallbackBackend,
        options,
      );
  ...
  private async scanWithBackend(
    ...
    try {
      switch (backend) {
        ...
        case "virustotal":
          return await this.scanWithVirusTotal(
            buffer,
            options.fileName,
            timeout,
          );
        case "plugin":
          return await this.scanWithPlugin(
            buffer,
            options.fileName,
            options.fileId,
            timeout,
  ...
  private async scanWithVirusTotal(
    ...
    const { apiKey, apiUrl, waitForResult, maxWaitTime } =
      this.config.virustotal;
  ...
  private async checkVirusTotalHealth(): Promise<void> {
    ...
    this.health.details = {
      quotaRemaining:
        data.data?.attributes?.api_requests_monthly?.user?.allowed,
    };
  }

...
describe("VirusScannerService", () => {
  ...
  describe("VirusScannerService.quarantineFile", () => {
    it("should quarantine infected file when enabled", async () => {
      ...
      const result = await scanner.quarantineFile(
        "file-123",
        "uploads/test.exe",
        ["Trojan.Win32"],
      );
  ...
  describe("toScanResult", () => {
    it("should convert VirusScanResult to ScanResult", () => {
      ...
      expect(scanResult.threatNames).toEqual([
        "Trojan.Generic",
        "Malware.Win32",
      ]);
...
describe("EICAR test virus detection", () => {
  ...
  it("EICAR string produces a File that can be passed to the scanner", () => {
    const eicarFile = new File([EICAR_TEST_STRING], "eicar.com", {
      type: "text/plain",
    });
  ...
  async function runClamAVScan(
    ...
    MockSocket.mockImplementation(() => {
      const record = {
        dataCallback: null as ((data: Buffer) => void) | null,
      ...
      return {
        connect: jest
          .fn()
          .mockImplementation((_p: number, _h: string, cb: () => void) => {
            setTimeout(cb, 0);
          }),
        write: jest.fn(),
        on: jest
          ...
          .mockImplementation(
            (event: string, cb: (...args: unknown[]) => void) => {
              if (event === "data")
                record.dataCallback = cb as (data: Buffer) => void;
    ...
    if (!scanSocket) {
      throw new Error(
        `Scan socket not created (sockets.length=${sockets.length}, preCount=${preCount})`,
      );
    }

    // Emit the scripted clamd response on the scan socket only.
    if (scanSocket.dataCallback)
      scanSocket.dataCallback(Buffer.from(clamdResponse));
...
describe("scanner-down default-reject behaviour", () => {
  ...
  it("scanner error with blockOnScannerUnavailable=true returns shouldBlock=true", async () => {
    ...
    MockSocket.mockImplementation(() => ({
      connect: jest
        .fn()
        .mockImplementation((_port: number, _host: string, _cb: () => void) => {

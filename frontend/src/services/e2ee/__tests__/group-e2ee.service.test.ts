  ...
  describe("distributeKeys", () => {
    it("should distribute keys to members", async () => {
      ...
      const members = service
        .getMembers("group-1")
        .filter((m) => m.userId !== "user-1");

      const result = await service.distributeKeys(
        "group-1",
        distributionMessage,
        members,
      );
...
describe("Group E2EE Service - Message Encryption/Decryption", () => {
    ...
    const mockAliceMessage = {
      ...
      encryptedData: btoa(
        JSON.stringify({
          ...
          chainKey: Array.from(aliceDistribution.chainKey)
            .map((b) => String.fromCharCode(b))
            .join(""),
          chainIteration: aliceDistribution.chainIteration,
          signingPublicKey: Array.from(aliceDistribution.signingPublicKey)
            .map((b) => String.fromCharCode(b))
            .join(""),
  ...
  describe("canDecryptFrom", () => {
    it("should return false when key is missing", () => {
      expect(
        bobService.canDecryptFrom("group-1", "alice", "device-a", 123),
      ).toBe(false);
    });
  });
});
...
describe("Group E2EE Service - Error Handling", () => {
  ...
  it("should throw when not initialized", async () => {
    ...
    expect(() => uninitializedService.getGroupInfo("group-1")).toThrow(
      "not initialized",
    );
  });
  ...
  it("should throw when decrypting without sender key", async () => {
    ...
    await expect(service.decryptMessage(fakeMessage)).rejects.toThrow(
      "Missing sender key",
    );
  });
});

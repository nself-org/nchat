  ...
  async handleCallback(params: URLSearchParams): Promise<AuthResult> {
    ...
    if (error) {
      return this.createErrorResult(
        this.createError(
          "OAUTH_ERROR",
          params.get("error_description") || "Google authentication failed",
        ),
      );
    }
      ...
      try {
        ...
        if (!response.ok) {
          return this.createErrorResult(
            this.createError(
              "AUTH_FAILED",
              data.error?.message || "Failed to authenticate with Google",
            ),
          );
        }
    ...
    if (code) {
      try {
        const response = await fetch(
          `${this.getAuthApiUrl()}/signin/provider/google/callback`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({

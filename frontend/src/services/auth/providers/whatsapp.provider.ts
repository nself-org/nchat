...
export class WhatsAppProvider extends BaseAuthProvider {
  ...
  async signIn(credentials: AuthCredentials): Promise<AuthResult> {
    if (!this.isEnabled()) {
      return this.createErrorResult(
        this.createError(
          "PROVIDER_DISABLED",
          "WhatsApp authentication is not enabled",
        ),
      );
    }
  ...
  async linkAccount(existingUserId: string): Promise<AuthResult> {
    return this.createErrorResult(
      this.createError(
        "NOT_IMPLEMENTED",
        "WhatsApp account linking not yet implemented",
      ),
    );
  }
  ...
  private async verifyCode(
    ...
    try {
      const response = await fetch(
        `${this.getAuthApiUrl()}/signin/whatsapp/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  private mapUserResponse(
    ...
    return {
      ...
      email:
        ...
        `${phoneNumber.replace("+", "")}@whatsapp.placeholder`,

 ...
 * - Access to Facebook profile and email
...
export interface FacebookConfig extends AuthProviderConfig {
  authApiUrl?: string;
...
export class FacebookProvider extends BaseAuthProvider {
  ...
  private extendedConfig: FacebookConfig = {
    enabled: false,
  ...
  async signIn(credentials: AuthCredentials): Promise<AuthResult> {
    if (!this.isEnabled()) {
      return this.createErrorResult(
        this.createError(
          "PROVIDER_DISABLED",
          "Facebook authentication is not enabled",
        ),
      );
    }
  ...
  async getAuthorizationUrl(): Promise<{ url: string; state: OAuthState }> {
    ...
    if (this.extendedConfig.authType) {
      params.append("auth_type", this.extendedConfig.authType);
    }

    return {
      url: `${this.getAuthApiUrl()}/signin/provider/facebook?${params.toString()}`,
      state,
    };
  }
    ...
    if (code) {
      try {
        const response = await fetch(
          `${this.getAuthApiUrl()}/signin/provider/facebook/callback`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  private getStoredSession(): {
    ...
    try {
      const stored = localStorage.getItem("nchat-facebook-session");
  ...
  private clearSession(): void {
    if (typeof window === "undefined") return;
    localStorage.removeItem("nchat-facebook-session");
  }

  private persistState(state: OAuthState): void {
    if (typeof window === "undefined") return;
    sessionStorage.setItem("nchat-facebook-oauth-state", JSON.stringify(state));
  }

  private clearState(): void {
    if (typeof window === "undefined") return;
    sessionStorage.removeItem("nchat-facebook-oauth-state");

...
describe("RoutingService", () => {
  ...
  async function createTestAgent(
    ...
    if (options.available !== false) {
      await livechatService.updateAgentStatus(
        agentResult.data!.id,
        "available",
      );
    }
  ...
  describe("Routing", () => {
    describe("routeConversation", () => {
      ...
      it("should list alternative agents", async () => {
        ...
        expect([agent1.id, agent2.id]).toContain(
          result.data?.alternativeAgents[0],
        );
      });
    });
    ...
    describe("Auto Selection (Round Robin)", () => {
      ...
      it("should use separate rotation per department", async () => {
        ...
        const supportConv = await createTestConversation({
          department: "support",
        });

        const salesResult = await routingService.routeConversation(salesConv);
        const supportResult =
          await routingService.routeConversation(supportConv);
    ...
    describe("Skill Based", () => {
      ...
      it("should prefer agents with matching skills", async () => {
        ...
        const billingAgent = await createTestAgent({
          skills: ["billing", "accounts"],
        });
    ...
    describe("Offline Action", () => {
      ...
      it("should send message when configured", async () => {
        ...
        const messagesResult = await livechatService.getMessages(
          conversation.id,
          {},
        );
  ...
  describe("Routing Rules", () => {
    ...
    describe("updateRule", () => {
      ...
      it("should return null for non-existent rule", () => {
        const result = routingService.updateRule("non-existent", {
          name: "Test",
        });
    ...
    describe("Rule Matching", () => {
      ...
      it("should match multiple conditions (AND)", async () => {
        ...
        const vipSpecialist = await createTestAgent({
          departments: ["vip"],
          priority: 2,
        });
        const vipBackup = await createTestAgent({
          departments: ["vip"],
          priority: 1,
        });
        ...
        const vipHighConv = await createTestConversation({
          department: "vip",
          priority: "high",
        });
        const vipHighResult =
          await routingService.routeConversation(vipHighConv);

        // Only matches department (not priority) - should fall through to normal routing
        const vipLowConv = await createTestConversation({
          department: "vip",
          priority: "low",
        });

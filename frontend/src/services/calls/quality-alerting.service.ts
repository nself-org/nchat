...
export class CallQualityAlertingService {
  ...
  checkThresholds(
    ...
    for (const threshold of this.config.thresholds) {
      ...
      if (exceeded) {
        ...
        const alert = this.createAlert(alertType, threshold.severity, context, {
   ...
   */
  private checkThresholdExceeded(
    value: number,
  ...
  async sendAlert(alert: Alert): Promise<boolean> {
    ...
    try {
      ...
      const sendPromises = this.config.channels
        .filter((c) => c.enabled)
        .filter(
          (c) => !c.severityFilter || c.severityFilter.includes(alert.severity),
        )
  ...
  private async sendEmail(
    ...
    logger.info(
      "[QualityAlerting] Email alert logged (mail plugin required for delivery)",
      {
        recipients,
        severity: alert.severity,
        title: alert.type,
          ...
          attachments: [
            {
              ...
              fields: [
                ...
                {
                  title: "Call ID",
                  value: alert.callId.substring(0, 8),
                  short: true,
                },
                {
                  title: "Suggestions",
  ...
  async getAlertHistory(filters: {
    ...
    try {
      ...
      const totalCount =
        data?.nchat_call_quality_alerts_aggregate?.aggregate?.count || 0;
      ...
      const alerts: Alert[] = dbAlerts.map((a: Record<string, unknown>) => ({
        ...
        acknowledgedAt: a.acknowledged_at
          ? new Date(a.acknowledged_at as string)
          : undefined,
        resolvedAt: a.resolved_at
          ? new Date(a.resolved_at as string)
          : undefined,
        escalatedAt: a.escalated_at
          ? new Date(a.escalated_at as string)
          : undefined,
      }));
    ...
    const avgResponseTime =
      acknowledgedAlerts.length > 0
        ? acknowledgedAlerts.reduce(
            (sum, a) =>
              sum + (a.acknowledgedAt!.getTime() - a.createdAt.getTime()),
            0,
   ...
   */
  suppressAlertType(callId: string, type: AlertType, durationMs: number): void {

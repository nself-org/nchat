...
describe("Offline + Sync + Cache Integration", () => {
  ...
  describe("Cache Management", () => {
    it("should cache messages for offline access", () => {
      ...
      localStorage.setItem(
        "cache-messages-channel-1",
        JSON.stringify(messages),
      );

      const cached = JSON.parse(
        localStorage.getItem("cache-messages-channel-1") || "[]",
      );
  ...
  describe("Cross-Module State Consistency", () => {
    it("should sync cache with sync queue state", () => {
      ...
      // Add to sync queue
      const syncQueue = [
        { id: message.id, type: "create_message", synced: false },
      ];
      localStorage.setItem("sync-queue", JSON.stringify(syncQueue));

      const cachedMessage = JSON.parse(
        localStorage.getItem(`cache-message-${message.id}`) || "{}",
      );
  ...
  describe("Conflict Resolution", () => {
    ...
    it("should resolve conflicts with last-write-wins", () => {
      ...
      const resolved =
        serverUpdate.updatedAt > localUpdate.updatedAt
          ? serverUpdate
          : localUpdate;
  ...
  describe("Background Sync", () => {
    it("should register background sync task", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem("background-sync-task") || "{}",
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should recover from corrupted cache", () => {
      ...
      const recovered = JSON.parse(
        localStorage.getItem("cache-messages") || "[]",
      );
  ...
  describe("Security", () => {
    ...
    it("should validate data before syncing", () => {
      ...
      const sanitized = operation.data.content.replace(
        /<script[^>]*>.*?<\/script>/gi,
        "",
      );

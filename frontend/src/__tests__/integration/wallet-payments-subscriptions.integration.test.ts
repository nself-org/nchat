...
describe("Wallet + Payments + Subscriptions Integration", () => {
  ...
  describe("Wallet Connection", () => {
    it("should connect wallet and store address", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`wallet-${mockUserId}`) || "{}",
      );
  ...
  describe("Payment Processing", () => {
    it("should create payment transaction", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`payment-${payment.id}`) || "{}",
      );
  ...
  describe("Subscription Management", () => {
    it("should create subscription", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`subscription-${mockUserId}`) || "{}",
      );
    ...
    it("should track subscription billing history", () => {
      ...
      expect(stored.every((b: { status: string }) => b.status === "paid")).toBe(
        true,
      );
    });
  });
  ...
  describe("Wallet Balance", () => {
    ...
    it("should update balance after transaction", () => {
      ...
      const newBalance = (
        parseFloat(wallet.balance) - parseFloat(transactionAmount)
      ).toFixed(1);
  ...
  describe("Payment Methods", () => {
    it("should support multiple payment methods", () => {
      ...
      expect(
        stored.filter((m: { type: string }) => m.type === "crypto"),
      ).toHaveLength(2);
    });
  ...
  describe("Refunds", () => {
    ...
    it("should handle partial refund", () => {
      ...
      payment.refunded = (
        parseFloat(payment.refunded) + parseFloat(refundAmount)
      ).toFixed(2);

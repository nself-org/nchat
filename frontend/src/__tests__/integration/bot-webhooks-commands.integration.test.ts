...
describe("Bot SDK + Webhooks + Commands Integration", () => {
  ...
  describe("Bot Registration and Setup", () => {
    it("should register bot with commands and webhook URL", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`bot-${mockBotId}`) || "{}",
      );
    ...
    it("should register bot event subscriptions", () => {
      ...
      localStorage.setItem(
        `bot-subscriptions-${mockBotId}`,
        JSON.stringify(bot.subscriptions),
      );

      const stored = JSON.parse(
        localStorage.getItem(`bot-subscriptions-${mockBotId}`) || "[]",
      );
  ...
  describe("Command Registration and Parsing", () => {
    it("should register commands with parameters", () => {
      ...
      localStorage.setItem(
        `bot-commands-${mockBotId}`,
        JSON.stringify(commands),
      );

      const stored = JSON.parse(
        localStorage.getItem(`bot-commands-${mockBotId}`) || "[]",
      );
    ...
    it("should handle command aliases", () => {
      ...
      const fullCommand =
        commandAliases[commandName as keyof typeof commandAliases] ||
        commandName;
  ...
  describe("Webhook Delivery", () => {
    ...
    it("should track webhook delivery history", () => {
      ...
      localStorage.setItem(
        `webhook-history-${mockBotId}`,
        JSON.stringify(deliveryHistory),
      );

      const stored = JSON.parse(
        localStorage.getItem(`webhook-history-${mockBotId}`) || "[]",
      );
      expect(stored).toHaveLength(3);

      const successRate =
        stored.filter((d: { status: number }) => d.status === 200).length /
        stored.length;
  ...
  describe("Bot Response Handling", () => {
    it("should send bot response to channel", () => {
      ...
      localStorage.setItem(
        `bot-message-${mockChannelId}`,
        JSON.stringify([botResponse]),
      );

      const stored = JSON.parse(
        localStorage.getItem(`bot-message-${mockChannelId}`) || "[]",
      );
  ...
  describe("Cross-Module State Consistency", () => {
    it("should sync bot state across webhook and command handlers", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`bot-state-${mockBotId}`) || "{}",
      );
    ...
    it("should maintain command execution history", () => {
      ...
      localStorage.setItem(
        `command-history-${mockBotId}`,
        JSON.stringify(commandHistory),
      );

      const stored = JSON.parse(
        localStorage.getItem(`command-history-${mockBotId}`) || "[]",
      );
      expect(stored).toHaveLength(3);

      const successRate =
        stored.filter((c: { success: boolean }) => c.success).length /
        stored.length;
      expect(successRate).toBeCloseTo(0.666, 2);
    });

...
describe("WebRTC + Calls Integration", () => {
  ...
  describe("Call Initiation and Signaling", () => {
    it("should initiate 1-on-1 call with signaling", async () => {
      ...
      localStorage.setItem(
        `signaling-${mockCallId}-offer`,
        JSON.stringify(signalingMessage),
      );

      const stored = JSON.parse(
        localStorage.getItem(`signaling-${mockCallId}-offer`) || "{}",
      );
    ...
    it("should answer incoming call", async () => {
      ...
      localStorage.setItem(
        `signaling-${mockCallId}-answer`,
        JSON.stringify(signalingMessage),
      );

      const stored = JSON.parse(
        localStorage.getItem(`signaling-${mockCallId}-answer`) || "{}",
      );
    ...
    it("should handle ICE connection states", () => {
      ...
      states.forEach((state) => {
        ...
        const stored = JSON.parse(
          localStorage.getItem(`connection-state-${mockCallId}`) || "{}",
        );
  ...
  describe("Group Calls", () => {
    it("should create group call with multiple participants", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`call-${groupCall.id}`) || "{}",
      );
    ...
    it("should remove participant from call", () => {
      ...
      groupCall.participants = groupCall.participants.filter(
        (p) => p !== leavingParticipant,
      );
  ...
  describe("Screen Sharing", () => {
    ...
    it("should stop screen sharing", async () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem("screen-share-stopped") || "{}",
      );
  ...
  describe("Call Recording", () => {
    it("should start recording call", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`recording-${mockCallId}`) || "{}",
      );
    ...
    it("should generate recording thumbnail", () => {
      ...
      localStorage.setItem(
        `thumbnail-${mockCallId}`,
        JSON.stringify(thumbnail),
      );

      const stored = JSON.parse(
        localStorage.getItem(`thumbnail-${mockCallId}`) || "{}",
      );
    ...
    it("should notify participants when recording starts", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem("recording-notification") || "{}",
      );
  ...
  describe("Call Quality Monitoring", () => {
    ...
    it("should detect poor connection quality", () => {
      ...
      const isPoorQuality =
        lossPercentage > 5 || stats.jitter > 0.1 || stats.rtt > 300;
  ...
  describe("Call State Management", () => {
    ...
    it("should decline incoming call", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`call-${mockCallId}`) || "{}",
      );
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle signaling failures", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem("signaling-error") || "{}",
      );
    ...
    it("should handle bandwidth limitations", () => {
      ...
      const shouldDisableVideo =
        stats.availableBandwidth < stats.requiredBandwidth;

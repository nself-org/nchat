...
describe("File Upload + Storage + Media Integration", () => {
  ...
  describe("File Validation", () => {
    ...
    it("should validate allowed MIME types", () => {
      const validTypes = [
        ...
        {
          file: new MockFile("exe.exe", 1024, "application/x-msdownload"),
          expected: false,
        },
      ];
  ...
  describe("File Upload Flow", () => {
    ...
    it("should preserve file metadata during upload", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`file-${metadata.id}`) || "{}",
      );
  ...
  describe("Storage Integration", () => {
    ...
    it("should generate storage URL after successful upload", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`storage-${fileId}`) || "{}",
      );
    ...
    it("should support presigned URLs for private files", () => {
      ...
      const isPresigned =
        presignedUrl.includes("token=") && presignedUrl.includes("expires=");
  ...
  describe("Media Processing", () => {
    ...
    it("should extract metadata from media files", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`metadata-${metadata.fileId}`) || "{}",
      );

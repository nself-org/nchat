...
describe("Notifications + Push + Badges Integration", () => {
  ...
  describe("Notification Creation", () => {
    it("should create notification for new message", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`notification-${notification.id}`) || "{}",
      );
  ...
  describe("Badge Count Management", () => {
    it("should increment badge count for unread notifications", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`badge-${mockUserId}`) || '{"count":0}',
      );
  ...
  describe("Push Notification Subscription", () => {
    it("should register push subscription", () => {
      ...
      localStorage.setItem(
        `push-subscription-${mockUserId}`,
        JSON.stringify(subscription),
      );

      const stored = JSON.parse(
        localStorage.getItem(`push-subscription-${mockUserId}`) || "{}",
      );
    ...
    it("should handle multiple device subscriptions", () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`push-subscriptions-${mockUserId}`) || "[]",
      );
  ...
  describe("Notification Preferences", () => {
    it("should respect user notification preferences", () => {
      ...
      localStorage.setItem(
        `notif-prefs-${mockUserId}`,
        JSON.stringify(preferences),
      );

      const shouldNotify = (type: string): boolean => {
        const prefs = JSON.parse(
          localStorage.getItem(`notif-prefs-${mockUserId}`) || "{}",
        );
    ...
    it("should support per-channel notification settings", () => {
      ...
      localStorage.setItem(
        `channel-notif-prefs-${mockUserId}`,
        JSON.stringify(channelPreferences),
      );

      const stored = JSON.parse(
        localStorage.getItem(`channel-notif-prefs-${mockUserId}`) || "{}",
      );
    ...
    it("should support notification sound preferences", () => {
      ...
      localStorage.setItem(
        `sound-prefs-${mockUserId}`,
        JSON.stringify(soundPreferences),
      );

      const stored = JSON.parse(
        localStorage.getItem(`sound-prefs-${mockUserId}`) || "{}",
      );
  ...
  describe("Platform-Specific Badge Updates", () => {
    ...
    it("should update document title badge", () => {
      ...
      const newTitle =
        badgeCount > 0 ? `(${badgeCount}) ${originalTitle}` : originalTitle;

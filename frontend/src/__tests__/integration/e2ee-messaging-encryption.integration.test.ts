...
describe("E2EE + Messaging Integration", () => {
  ...
  describe("Key Generation and Storage", () => {
    it("should generate identity key pair for new user", async () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem(`e2ee-identity-${mockUserId1}`) || "{}",
      );
    ...
    it("should generate prekey bundle for key exchange", async () => {
      const prekeyBundle = {
        ...
        oneTimePreKeys: ["alice-otpk-1", "alice-otpk-2", "alice-otpk-3"],
      };
      ...
      const stored = JSON.parse(
        localStorage.getItem(`prekey-bundle-${mockUserId1}`) || "{}",
      );
  ...
  describe("X3DH Key Exchange", () => {
    it("should establish shared secret using X3DH", async () => {
      ...
      const session = JSON.parse(
        localStorage.getItem(`session-${mockUserId1}-${mockUserId2}`) || "{}",
      );
  ...
  describe("Double Ratchet Encryption", () => {
    it("should encrypt message using current chain key", async () => {
      ...
      const stored = JSON.parse(
        localStorage.getItem("encrypted-message") || "{}",
      );
    ...
    it("should perform DH ratchet on receiving new public key", async () => {
      ...
      if (newRatchetPublicKey !== session.receivingRatchetPublicKey) {
        ...
        const [newRootKey, newReceivingChainKey] = [
          `root-key-2`,
          `receiving-chain-2`,
        ];
  ...
  describe("Encrypted Message Flow", () => {
    ...
    it("should receive and decrypt message end-to-end", async () => {
      ...
      // Get session
      const session = JSON.parse(
        localStorage.getItem(`session-${mockUserId2}-${mockUserId1}`) || "{}",
      );
    ...
    it("should maintain message order after decryption", async () => {
      ...
      localStorage.setItem(
        "decrypted-messages",
        JSON.stringify(decryptedMessages),
      );

      const stored = JSON.parse(
        localStorage.getItem("decrypted-messages") || "[]",
      );
  ...
  describe("Group Chat Encryption", () => {
    it("should create sender key for group", async () => {
      ...
      localStorage.setItem(
        `sender-key-${mockUserId1}-${groupId}`,
        JSON.stringify(senderKey),
      );

      const stored = JSON.parse(
        localStorage.getItem(`sender-key-${mockUserId1}-${groupId}`) || "{}",
      );
    ...
    it("should distribute sender key to group members", async () => {
      ...
      members.forEach((memberId) => {
        ...
        localStorage.setItem(
          `sender-key-distribution-${memberId}`,
          encryptedSenderKey,
        );
      });

      expect(
        localStorage.getItem(`sender-key-distribution-${mockUserId2}`),
      ).toBeDefined();
      expect(
        localStorage.getItem(`sender-key-distribution-${mockUserId3}`),
      ).toBeDefined();
    });
    ...
    it("should handle member removal from group", async () => {
      ...
      expect(
        localStorage.getItem(`sender-key-v2-${removedMemberId}`),
      ).toBeNull();
    });
  });
  ...
  describe("Encrypted File Attachments", () => {
    ...
    it("should encrypt file using AES-GCM", async () => {
      ...
      mockCrypto.subtle.encrypt.mockResolvedValueOnce(
        new ArrayBuffer(file.length + 16),
      );
  ...
  describe("Key Rotation and Compromise", () => {
    ...
    it("should re-establish all sessions after key rotation", () => {
      ...
      expect(
        localStorage.getItem(`session-${mockUserId1}-${mockUserId3}`),
      ).toBeNull();
    });
  ...
  describe("Safety Numbers and Verification", () => {
    it("should compute safety number from identity keys", () => {
      const aliceIdentityKey =
        "alice-identity-public-key-1234567890123456789012345678";
      const bobIdentityKey =
        "bob-identity-public-key-0987654321098765432109876543";

      const combined = `${aliceIdentityKey}${bobIdentityKey}`
        .split("")
        .sort()
        .join("");
    ...
    it("should mark conversation as verified", () => {
      ...
      const verification = JSON.parse(
        localStorage.getItem(`verified-${conversationId}`) || "{}",
      );
    ...
    it("should warn on identity key change", () => {
      ...
      const warning = JSON.parse(
        localStorage.getItem("security-warning") || "{}",
      );
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle decryption failure gracefully", async () => {
      ...
      const errorLog = JSON.parse(
        localStorage.getItem("decryption-error") || "{}",
      );

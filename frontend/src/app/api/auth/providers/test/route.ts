...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Insufficient permissions. Only admins and owners can test providers.",
        },
        { status: 403 },
      );
    }
    ...
    const summary = {
      ...
      notConfigured: results.filter((r) => r.status === "not_configured")
        .length,
    };
...
async function testTwitterProvider(): Promise<ProviderTestResult> {
  ...
  return {
    ...
    status: "working",
    message:
      "Twitter/X OAuth credentials are configured (connectivity not verified)",
    details: {

...
export default function AnalyticsPage() {
  ...
  const chartData = useMemo(() => {
    ...
    const byDay: Record<
      string,
      { date: string; users: number; messages: number; activeUsers: number }
    > = {};
    ...
    for (const user of analyticsData.user_signups ?? []) {
      ...
      if (!byDay[day])
        byDay[day] = { date: day, users: 0, messages: 0, activeUsers: 0 };
      byDay[day].users += 1;
    }
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      result.push(
        byDay[key] ?? { date: key, users: 0, messages: 0, activeUsers: 0 },
      );
    }
    return result;
  }, [analyticsData, days]);
  ...
  const popularChannels = useMemo(() => {
    ...
    const channels = [
      ...(analyticsData.active_channels as Array<{
        id: string;
        name: string;
    ...
    channels.sort(
      (a, b) =>
        b.messages_aggregate.aggregate.count -
        a.messages_aggregate.aggregate.count,
    );
    ...
    return channels.slice(0, 5).map((ch) => ({
      ...
      percentage: Math.round(
        (ch.messages_aggregate.aggregate.count / maxMessages) * 100,
      ),
    }));
  }, [analyticsData]);
  ...
  const roleDistribution = useMemo(() => {
    ...
    return (
      analyticsData.role_distribution as Array<{
        id: string;
        name: string;
  ...
  const peakHours = useMemo(() => {
    ...
    return Object.entries(hourBuckets)
      ...
      .sort(
        (a, b) => HOUR_LABELS.indexOf(a.hour) - HOUR_LABELS.indexOf(b.hour),
      );
  }, [analyticsData]);

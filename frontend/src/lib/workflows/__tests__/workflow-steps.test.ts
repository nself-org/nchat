...
describe("validateStep", () => {
  ...
  it("errors on trigger without triggerType", () => {
    ...
    expect(errs.some((e) => /Trigger type is required/.test(e.message))).toBe(
      true,
    );
  });
  ...
  it("errors on message step with empty content", () => {
    ...
    expect(
      errs.some((e) => /Message content is required/.test(e.message)),
    ).toBe(true);
  });
  it("warns on message step channel target without channelId", () => {
    const m = createMessageStep(
      { x: 0, y: 0 },
      { content: "hi", target: "channel" },
    );
    const errs = validateStep(m);
    expect(
      errs.some(
        (e) =>
          e.severity === "warning" && /Channel is required/.test(e.message),
      ),
    ).toBe(true);
  });
  ...
  it("errors on delay fixed with invalid duration", () => {
    ...
    expect(
      errs.some((e) => /Duration must be greater than 0/.test(e.message)),
    ).toBe(true);
  });
  it("errors on delay until_time missing untilTime", () => {
    const d = createDelayStep({ x: 0, y: 0 }, { delayType: "until_time" });
    const errs = validateStep(d);
    expect(errs.some((e) => /Target time is required/.test(e.message))).toBe(
      true,
    );
  });
  it("errors on webhook with empty url", () => {
    ...
    expect(errs.some((e) => /Webhook URL is required/.test(e.message))).toBe(
      true,
    );
  });
  ...
  it("errors on approval without approvers or roles", () => {
    const a = createApprovalStep(
      { x: 0, y: 0 },
      { approvers: [], approverRoles: [] },
    );
    const errs = validateStep(a);
    expect(errs.some((e) => /approver or approver role/.test(e.message))).toBe(
      true,
    );
  });
  it("errors on approval with empty message", () => {
    ...
    const errs = validateStep(a);
    expect(
      errs.some((e) => /Approval message is required/.test(e.message)),
    ).toBe(true);
  });
  it("errors on loop for_each missing collection", () => {
    ...
    expect(
      errs.some((e) => /Collection variable is required/.test(e.message)),
    ).toBe(true);
  });
  it("errors on loop count with invalid count", () => {
    ...
    const errs = validateStep(l);
    expect(
      errs.some((e) => /Count must be greater than 0/.test(e.message)),
    ).toBe(true);
  });
  it("errors on loop with maxIterations below 1", () => {
    const l = createLoopStep(
      { x: 0, y: 0 },
      { loopType: "count", count: 5, maxIterations: -1 },
    );
    const errs = validateStep(l);
    expect(
      errs.some((e) => /Maximum iterations must be at least 1/.test(e.message)),
    ).toBe(true);
  });
  it("errors on parallel step with fewer than 2 branches", () => {
    const p = createParallelStep(
      { x: 0, y: 0 },
      { branches: [{ id: "b1", name: "A" }] as any },
    );
...
describe("step utilities", () => {
  ...
  it("getStepsByCategory returns matching types", () => {
    ...
    expect(steps).toEqual(
      expect.arrayContaining(["condition", "loop", "parallel"]),
    );
  });
  ...
  it("getOutputHandles returns correct values per type", () => {
    ...
    expect(getOutputHandles("approval")).toEqual([
      "approved",
      "rejected",
      "timeout",
    ]);

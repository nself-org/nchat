...
describe("validateWorkflow", () => {
  it("errors when name is empty", () => {
    ...
    expect(res.errors.some((e) => /name is required/i.test(e.message))).toBe(
      true,
    );
  });
  ...
  it("detects edge referencing missing source", () => {
    ...
    const res = validateWorkflow(wf);
    expect(res.errors.some((e) => /non-existent source/.test(e.message))).toBe(
      true,
    );
  });
  it("detects edge referencing missing target", () => {
    ...
    const res = validateWorkflow(wf);
    expect(res.errors.some((e) => /non-existent target/.test(e.message))).toBe(
      true,
    );
  });
  it("flags invalid variable name as error", () => {
    ...
    const res = validateWorkflow(wf);
    expect(
      res.errors.some((e) => /must start with a letter/.test(e.message)),
    ).toBe(true);
  });
  it("flags duplicate variable names", () => {
    ...
    const res = validateWorkflow(wf);
    expect(res.errors.some((e) => /Duplicate variable/.test(e.message))).toBe(
      true,
    );
  });
...
describe("workflow runs", () => {
  ...
  it("updateRunStatus sets completedAt on terminal states", () => {
    ...
    const failed = updateRunStatus(run, "failed", {
      code: "E",
      message: "boom",
    });
...
describe("findMatchingWorkflows", () => {
  it("skips inactive workflows", () => {
    ...
    wf = addStep(
      wf,
      mkTrigger("t1", { config: { triggerType: "manual" } as any }),
    );
...
describe("serialization", () => {
  ...
  it("deserializeWorkflow throws on malformed json", () => {
    expect(() => deserializeWorkflow('{"nope":true}')).toThrow(
      /Invalid workflow/,
    );
  });
});

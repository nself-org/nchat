...
describe("euclideanDistance", () => {
  ...
  it("is symmetric", () => {
    expect(euclideanDistance([1, 2], [5, 6])).toBe(
      euclideanDistance([5, 6], [1, 2]),
    );
  });
});
...
describe("weightedAverageVectors", () => {
  it("weights: equal weights == plain average", () => {
    const equal = weightedAverageVectors(
      [
        [1, 0],
        [0, 1],
      ],
  ...
  it("bias toward higher-weight vector", () => {
    const r = weightedAverageVectors(
      [
        [1, 0],
        [0, 1],
      ],
  ...
  it("throws on mismatched weight count", () => {
    expect(() => weightedAverageVectors([[1]], [1, 2])).toThrow(
      /match number of weights/i,
    );
  });
  it("throws on zero total weight", () => {
    expect(() => weightedAverageVectors([[1], [2]], [0, 0])).toThrow(
      /weight cannot be zero/i,
    );
  });
  it("throws on dimension mismatch", () => {
    expect(() => weightedAverageVectors([[1, 2], [1]], [1, 1])).toThrow(
      /same dimension/i,
    );
  });
});

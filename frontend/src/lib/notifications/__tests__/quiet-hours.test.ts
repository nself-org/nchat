...
const baseSchedule = (
  overrides: Partial<QuietHoursSchedule> = {},
): QuietHoursSchedule => ({
  enabled: true,
  startTime: "22:00",
  endTime: "08:00",
...
describe("isInQuietHours", () => {
  ...
  it("ignoreWeekendSettings bypasses enableOnWeekends", () => {
    ...
    expect(
      isInQuietHours(s, { checkDate: d2, ignoreWeekendSettings: true }),
    ).toBe(true);
  });
});
...
describe("validateQuietHoursSchedule", () => {
  ...
  it("flags out-of-range day", () => {
    const r = validateQuietHoursSchedule(
      baseSchedule({ days: [7 as DayOfWeek] }),
    );
    expect(r.valid).toBe(false);
  });
  it("flags invalid timezone", () => {
    const r = validateQuietHoursSchedule(
      baseSchedule({ timezone: "Not/A_Zone" }),
    );
    expect(r.valid).toBe(false);
  });
});
...
describe("getNextQuietHoursPeriod", () => {
  it("returns null when disabled", () => {
    expect(
      getNextQuietHoursPeriod(baseSchedule({ enabled: false })),
    ).toBeNull();
  });
...
describe("isInWeekendQuietHours", () => {
  ...
  it("returns false when disabled", () => {
    expect(
      isInWeekendQuietHours(
        { enabled: false, startTime: "00:00", endTime: "12:00" },
        baseSchedule(),
      ),
    ).toBe(false);
  });

...
describe("matchKeyword", () => {
  ...
  it("respects wholeWord", () => {
    const r = matchKeyword(
      "alerting scatter alert",
      kw({ keyword: "alert", wholeWord: true }),
    );
    expect(r).toHaveLength(1);
  });
  it("respects maxMatches", () => {
    const r = matchKeyword("a a a a a", kw({ keyword: "a", wholeWord: true }), {
      maxMatches: 2,
    });
    expect(r).toHaveLength(2);
  });
...
describe("matchKeywords and hasKeywordMatch", () => {
  it("sorts matches by position", () => {
    ...
    const r = matchKeywords(text, [
      kw({ id: "a", keyword: "alert" }),
      kw({ id: "b", keyword: "bug" }),
    ]);
...
describe("highlightMatches and getHighlightedResult", () => {
  ...
  it("uses custom tag and class", () => {
    ...
    const h = highlightMatches("alert", matches, {
      highlightTag: "span",
      highlightClass: "hi",
    });
...
describe("isDuplicateKeyword", () => {
  ...
  it("respects caseSensitive option", () => {
    expect(
      isDuplicateKeyword("ALERT", [kw({ keyword: "alert" })], {
        caseSensitive: true,
      }),
    ).toBe(false);
  });
...
describe("sortKeywords and searchKeywords", () => {
  it("sorts alphabetically ascending", () => {
    const r = sortKeywords([
      kw({ id: "b", keyword: "bravo" }),
      kw({ id: "a", keyword: "alpha" }),
    ]);
  ...
  it("searchKeywords filters by term", () => {
    const r = searchKeywords(
      [kw({ keyword: "alert" }), kw({ keyword: "bug" })],
      "al",
    );
    expect(r).toHaveLength(1);

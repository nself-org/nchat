...
describe("shouldBypassDigest", () => {
  it("bypasses when realtime", () => {
    expect(
      shouldBypassDigest(
        createDigestConfig({ frequency: "realtime", enabled: true }),
        entry(),
      ),
    ).toBe(true);
  });
  it("bypasses when disabled", () => {
    expect(
      shouldBypassDigest(createDigestConfig({ enabled: false }), entry()),
    ).toBe(true);
  });
...
describe("shouldSendDigest", () => {
  it("false when disabled", () => {
    expect(
      shouldSendDigest(
        createDigestConfig({ enabled: false }),
        createDeliveryState(),
      ),
    ).toBe(false);
  });
  it("false when realtime", () => {
    expect(
      shouldSendDigest(
        createDigestConfig({ enabled: true, frequency: "realtime" }),
        createDeliveryState(),
      ),
    ).toBe(false);
  });
  it("false when pending count below minimum", () => {
    ...
    expect(
      shouldSendDigest(
        createDigestConfig({ enabled: true, minimumBatchSize: 3 }),
        s,
      ),
    ).toBe(false);
  });
...
describe("getNextDigestTime", () => {
  it("returns null when disabled", () => {
    expect(
      getNextDigestTime(createDigestConfig({ enabled: false })),
    ).toBeNull();
  });
  ...
  it("returns future Date for hourly", () => {
    ...
    const next = getNextDigestTime(
      createDigestConfig({ enabled: true, frequency: "hourly" }),
      undefined,
      now,
    );
...
describe("generateDigest", () => {
  ...
  it("filters read notifications when includeRead=false", () => {
    ...
    const entries = [
      entry({ id: "r", isRead: true }),
      entry({ id: "u", isRead: false }),
    ];
    const d = generateDigest(entries, c);

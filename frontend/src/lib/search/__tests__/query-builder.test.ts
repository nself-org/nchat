...
describe("SearchQueryBuilder basic filters", () => {
  ...
  it("dateRange after only", () => {
    const b = new SearchQueryBuilder("x").dateRange({
...
describe("attachment and message flags", () => {
  ...
  it("hasLink adds filter", () => {
    expect(new SearchQueryBuilder("x").hasLink().getFilterString()).toContain(
      "has_link = true",
    );
  });
  it("isPinned toggle", () => {
    expect(
      new SearchQueryBuilder("x").isPinned(false).getFilterString(),
    ).toContain("is_pinned = false");
  });
  it("isEdited", () => {
    expect(new SearchQueryBuilder("x").isEdited().getFilterString()).toContain(
      "is_edited = true",
    );
  });
  it("inThread with id", () => {
    expect(
      new SearchQueryBuilder("x").inThread("t1").getFilterString(),
    ).toContain('parent_thread_id = "t1"');
  });
  it("inThread without id", () => {
    expect(new SearchQueryBuilder("x").inThread().getFilterString()).toContain(
      "parent_thread_id EXISTS",
    );
  });
  it("rootMessagesOnly", () => {
    expect(
      new SearchQueryBuilder("x").rootMessagesOnly().getFilterString(),
    ).toContain("NOT EXISTS");
  });
  it("hasThread", () => {
    expect(new SearchQueryBuilder("x").hasThread().getFilterString()).toContain(
      "thread_id EXISTS",
    );
  });
  it("mentioning single", () => {
    expect(
      new SearchQueryBuilder("x").mentioning("u1").getFilterString(),
    ).toContain('mentioned_users = "u1"');
  });
  it("mentionsEveryone/Here", () => {
    expect(
      new SearchQueryBuilder("x").mentionsEveryone().getFilterString(),
    ).toContain("mentions_everyone = true");
    expect(
      new SearchQueryBuilder("x").mentionsHere().getFilterString(),
    ).toContain("mentions_here = true");
  });
  it("ofType string and array", () => {
    expect(
      new SearchQueryBuilder("x").ofType("text").getFilterString(),
    ).toContain('message_type = "text"');
    const s = new SearchQueryBuilder("x")
      .ofType(["text", "file"])
      .getFilterString();
    expect(s).toContain("OR");
  });
  it("excludeDeleted", () => {
    expect(
      new SearchQueryBuilder("x").excludeDeleted().getFilterString(),
    ).toContain("is_deleted = false");
  });
  it("addFilter custom", () => {
    expect(
      new SearchQueryBuilder("x").addFilter("foo = true").getFilterString(),
    ).toContain("foo = true");
  });
});
...
describe("sort + paging + build", () => {
  ...
  it("highlight and crop overrides", () => {
    const opts = new SearchQueryBuilder("x")
      .highlight(["content"])
      .crop(["content"], 50)
      .build();
...
describe("parseSearchString", () => {
  it("parses from/in/has/is/before/after", () => {
    const r = parseSearchString(
      "hello from:alice in:#general has:file is:pinned before:2024-01-01 after:2023-01-01",
    );
  ...
  it("maps has:attachment to all three", () => {
    ...
    expect(r.hasAttachment).toEqual(
      expect.arrayContaining(["file", "image", "video"]),
    );
  });

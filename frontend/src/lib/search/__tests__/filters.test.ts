...
describe("buildWhereClause", () => {
  ...
  it("builds conditions for date range", () => {
    const r = new SearchFilterBuilder()
      .after(new Date("2024-01-01"))
      .before(new Date("2024-02-01"))
      .buildWhereClause();
...
describe("buildOrderByClause", () => {
  it("default relevance", () => {
    expect(new SearchFilterBuilder().buildOrderByClause()).toContain(
...
describe("FilterValidator", () => {
  ...
  it("similarity threshold out of range", () => {
    expect(FilterValidator.validate({ similarityThreshold: 1.5 }).valid).toBe(
      false,
    );
    expect(FilterValidator.validate({ similarityThreshold: -0.1 }).valid).toBe(
      false,
    );
  });

...
describe("matchesShortcut", () => {
  ...
  it("requires alt when shortcut has alt", () => {
    ...
    expect(matchesShortcut(kev({ key: "k", altKey: true }), "alt+k")).toBe(
      true,
    );
  });
  ...
  it("requires shift when shortcut has shift", () => {
    ...
    expect(matchesShortcut(kev({ key: "k", shiftKey: true }), "shift+k")).toBe(
      true,
    );
  });
...
describe("formatShortcut", () => {
  ...
  it("respects custom separator", () => {
    expect(
      formatShortcut("mod+k", { useMacSymbols: false, separator: "-" }),
    ).toContain("-");
  });

  it("handles non-uppercase", () => {
    expect(
      formatShortcut("mod+k", { useMacSymbols: false, uppercase: false }),
    ).toContain("k");
  });
});
...
describe("formatKeyArray / splitShortcutForDisplay", () => {
  it("formatKeyArray joins correctly", () => {
    expect(formatKeyArray(["mod", "k"], { useMacSymbols: false })).toContain(
      "K",
    );
  });
...
describe("eventToShortcut", () => {
  it("includes modifiers", () => {
    const out = eventToShortcut(
      kev({ key: "k", ctrlKey: !isMacOS(), metaKey: isMacOS() }),
    );
  ...
  it("includes alt/shift", () => {
    const out = eventToShortcut(
      kev({ key: "k", altKey: true, shiftKey: true }),
    );
...
describe("shouldIgnoreShortcut", () => {
  it("ignores when in input", () => {
    ...
    expect(shouldIgnoreShortcut(kev({ key: "k", target: input }) as any)).toBe(
      true,
    );
  });
  ...
  it("allows Escape in inputs", () => {
    const input = document.createElement("input");
    expect(
      shouldIgnoreShortcut(kev({ key: "Escape", target: input }) as any),
    ).toBe(false);
  });
  ...
  it("does not ignore when enableOnInputs", () => {
    const input = document.createElement("input");
    expect(
      shouldIgnoreShortcut(kev({ key: "k", target: input }) as any, {
        enableOnInputs: true,
      }),
    ).toBe(false);
  });
  ...
  it("ignores on div with no contenteditable", () => {
    ...
    expect(shouldIgnoreShortcut(kev({ key: "k", target: div }) as any)).toBe(
      false,
    );
  });
});

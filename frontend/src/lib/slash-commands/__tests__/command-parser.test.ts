...
describe("parseCommand - numeric types", () => {
  ...
  it("rejects non-numbers", () => {
    const r = parseCommand(
      ...
      cmd({
        arguments: [
          arg({ id: "n", type: "number", required: true, position: 0 }),
        ],
      }),
    );
    expect(r.isValid).toBe(false);
...
describe("parseCommand - booleans", () => {
  ...
  it("rejects invalid boolean", () => {
    const r = parseCommand(
      ...
      cmd({
        arguments: [
          arg({ id: "b", type: "boolean", required: true, position: 0 }),
        ],
      }),
    );
    expect(r.isValid).toBe(false);
...
describe("parseCommand - duration / date / time", () => {
  ...
  it("rejects invalid duration", () => {
    const r = parseCommand(
      ...
      cmd({
        arguments: [
          arg({ id: "d", type: "duration", required: true, position: 0 }),
        ],
      }),
    );
    expect(r.isValid).toBe(false);
...
describe("parseCommand - flags", () => {
  ...
  it("parses short flag", () => {
    const r = parseCommand(
      ...
      cmd({
        arguments: [
          arg({ id: "name", flag: "name", shortFlag: "n", type: "string" }),
        ],
      }),
    );
    expect(r.flags.name?.value).toBe("alice");
  ...
  it("reports missing required flag", () => {
    const r = parseCommand(
      ...
      cmd({
        arguments: [
          arg({ id: "name", flag: "name", type: "string", required: true }),
        ],
      }),
    );

...
describe("google/oauth", () => {
  ...
  describe("handleGoogleOAuthCallback", () => {
    ...
    it("rejects callback-level error", async () => {
      await expect(
        handleGoogleOAuthCallback(c, { error: "access_denied" }),
      ).rejects.toThrow(GoogleOAuthException);
    });

    it("rejects missing code or state", async () => {
      await expect(
        handleGoogleOAuthCallback(c, { state: "s" }),
      ).rejects.toThrow(/code/i);
      await expect(handleGoogleOAuthCallback(c, { code: "c" })).rejects.toThrow(
        /state/i,
      );
    });

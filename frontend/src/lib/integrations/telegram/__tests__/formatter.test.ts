...
describe("formatTelegramNotification — message branches", () => {
  ...
  it("edited_message", () => {
    const n = formatTelegramNotification({
      edited_message: mkMsg({ text: "edit" }),
    } as any);
  ...
  it("edited_channel_post", () => {
    const n = formatTelegramNotification({
      edited_channel_post: mkMsg({
        text: "x",
        chat: baseChat("channel", "News"),
      }),
    } as any);
  ...
  it("photo without caption", () => {
    const n = formatTelegramNotification({
      message: mkMsg({ photo: [{ file_id: "p1" }] }),
    } as any);
  ...
  it("document no filename", () => {
    expect(
      formatTelegramNotification({
        message: mkMsg({ document: { file_id: "d" } }),
      } as any).body,
    ).toBe("File received");
  });
  ...
  it("sticker no emoji", () => {
    expect(
      formatTelegramNotification({ message: mkMsg({ sticker: {} }) } as any)
        .body,
  ...
  it("location", () => {
    expect(
      formatTelegramNotification({ message: mkMsg({ location: {} }) } as any)
        .icon,
    ).toBe("location");
  });
  it("contact", () => {
  ...
  it("new chat members", () => {
    const n = formatTelegramNotification({
      message: mkMsg({
        new_chat_members: [{ first_name: "Alice" }, { first_name: "Bob" }],
      }),
    } as any);
  ...
  it("no recognized message type", () => {
    expect(
      formatTelegramNotification({ message: mkMsg({}) } as any).title,
    ).toBe("Message in Group");
  });

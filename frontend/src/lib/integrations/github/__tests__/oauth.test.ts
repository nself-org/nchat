...
describe("github/oauth", () => {
  ...
  describe("buildGitHubAuthUrl", () => {
    ...
    it("honors custom scopes", () => {
      const url = new URL(
        buildGitHubAuthUrl({ ...config, scopes: ["repo", "read:user"] }),
      );
    ...
    it("passes state and login options", () => {
      const url = new URL(
        buildGitHubAuthUrl(config, { state: "nn", login: "alice" }),
      );
  ...
  describe("exchangeCodeForToken (fetch wrapper)", () => {
    ...
    it("throws GitHubOAuthException on error payload", async () => {
      ...
      await expect(exchangeCodeForToken(config, "x")).rejects.toBeInstanceOf(
        GitHubOAuthException,
      );
    });
  ...
  describe("handleGitHubOAuthCallback", () => {
    ...
    it("rejects when callback has error", async () => {
      await expect(
        handleGitHubOAuthCallback(config, { error: "access_denied" }),
      ).rejects.toThrow(GitHubOAuthException);
    });

    it("rejects when missing code", async () => {
      await expect(
        handleGitHubOAuthCallback(config, { state: "s" }),
      ).rejects.toThrow(/missing/i);
    });

    it("rejects when missing state", async () => {
      await expect(
        handleGitHubOAuthCallback(config, { code: "c" }),
    ...
    it("succeeds when state valid and token exchange succeeds", async () => {
      ...
      const res = await handleGitHubOAuthCallback(config, {
        code: "code",
        state,
      });
  ...
  describe("scope utilities", () => {
    it("parseScopes splits on comma and whitespace", () => {
      expect(parseScopes("repo, user   admin:org")).toEqual([
        "repo",
        "user",
        "admin:org",
      ]);
    ...
    it("getMissingScopes lists only the absent ones", () => {
      expect(getMissingScopes("repo", ["repo", "user", "gist"])).toEqual([
        "user",
        "gist",
      ]);
    });

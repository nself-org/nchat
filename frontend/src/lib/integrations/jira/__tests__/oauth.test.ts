...
describe("jira/oauth", () => {
  ...
  describe("exchangeCodeForToken", () => {
    ...
    it("throws on error field", async () => {
      ...
      await expect(exchangeCodeForToken(c, "x")).rejects.toBeInstanceOf(
        JiraOAuthException,
      );
    });
    ...
    it("throws when access_token missing", async () => {
      ...
      await expect(exchangeCodeForToken(c, "x")).rejects.toThrow(
        JiraOAuthException,
      );
    });
  });
  ...
  describe("refreshAccessToken", () => {
    ...
    it("throws on error", async () => {
      ...
      await expect(refreshAccessToken(c, "x")).rejects.toThrow(
        JiraOAuthException,
      );
    });
  });
  ...
  describe("getAccessibleResources", () => {
    it("returns resource list on ok", async () => {
      const resources = [
        { id: "1", url: "https://x", name: "n", scopes: [], avatarUrl: "" },
      ];
      fetchMock.mockResolvedValueOnce({
        ok: true,
    ...
    it("throws when response not ok", async () => {
      ...
      await expect(getAccessibleResources("tok")).rejects.toThrow(
        JiraOAuthException,
      );
    });
  });
  ...
  describe("handleJiraOAuthCallback", () => {
    ...
    it("rejects on callback error", async () => {
      await expect(
        handleJiraOAuthCallback(c, { error: "access_denied" }),
      ).rejects.toThrow(JiraOAuthException);
    });

    it("rejects missing code/state", async () => {
      await expect(handleJiraOAuthCallback(c, { state: "s" })).rejects.toThrow(
        /code/i,
      );
      await expect(handleJiraOAuthCallback(c, { code: "c" })).rejects.toThrow(
        /state/i,
      );
    });
    ...
    it("rejects unknown state", async () => {
      await expect(
        handleJiraOAuthCallback(c, { code: "c", state: "nope" }),
    ...
    it("full flow success with resources", async () => {
      ...
      const fetchSpy = fetchMock
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => [
            { id: "1", url: "u", name: "n", scopes: [], avatarUrl: "" },
          ],
        } as Response);

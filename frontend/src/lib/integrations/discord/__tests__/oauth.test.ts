...
describe("discord/oauth", () => {
  ...
  describe("buildDiscordAuthUrl", () => {
    ...
    it("sets response_type=code and space-separated scopes", () => {
      const u = new URL(
        buildDiscordAuthUrl({ ...c, scopes: ["identify", "guilds"] }),
      );
    ...
    it("passes state and prompt", () => {
      const u = new URL(
        buildDiscordAuthUrl(c, { state: "nn", prompt: "none" }),
      );
  ...
  describe("handleDiscordOAuthCallback", () => {
    ...
    it("rejects callback error", async () => {
      await expect(
        handleDiscordOAuthCallback(c, { error: "access_denied" }),
      ).rejects.toThrow(DiscordOAuthException);
    });

    it("rejects missing code", async () => {
      await expect(
        handleDiscordOAuthCallback(c, { state: "s" }),
      ).rejects.toThrow(/code/i);
    });

    it("rejects missing state", async () => {
      await expect(
        handleDiscordOAuthCallback(c, { code: "c" }),
      ).rejects.toThrow(/state/i);
    });

    it("rejects invalid state", async () => {
      await expect(
        handleDiscordOAuthCallback(c, { code: "c", state: "nope" }),
    ...
    it("succeeds and passes through guildId", async () => {
      ...
      const res = await handleDiscordOAuthCallback(c, {
        code: "x",
        state,
        guildId: "G",
      });
  ...
  describe("exceptions and utils", () => {
    ...
    it("parseScopes splits on whitespace only (Discord convention)", () => {
      expect(parseScopes("identify  guilds email")).toEqual([
        "identify",
        "guilds",
        "email",
      ]);
  ...
  describe("calculatePermissions", () => {
    ...
    it("is idempotent for duplicate flags (bitmask OR)", () => {
      expect(calculatePermissions(["ADMINISTRATOR", "ADMINISTRATOR"])).toBe(
        "8",
      );
    });
  });

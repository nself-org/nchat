...
describe("formatSlackNotification", () => {
  ...
  it("channel events — all 5 branches", () => {
    expect(
      formatSlackNotification(wrap({ type: "channel_created", channel: "c" }))
        .color,
    ).toBe("green");
    expect(
      formatSlackNotification(wrap({ type: "channel_deleted", channel: "c" }))
        .color,
    ).toBe("red");
    expect(
      formatSlackNotification(wrap({ type: "channel_archive", channel: "c" }))
        .color,
    ).toBe("gray");
    expect(
      formatSlackNotification(wrap({ type: "channel_unarchive", channel: "c" }))
        .color,
    ).toBe("green");
    expect(
      formatSlackNotification(wrap({ type: "channel_rename", channel: "c" }))
        .color,
    ).toBe("blue");
  });
  it("file events — all 3 branches", () => {
    expect(
      formatSlackNotification(wrap({ type: "file_created", channel: "c" }))
        .color,
    ).toBe("green");
    expect(
      formatSlackNotification(wrap({ type: "file_shared", channel: "c" }))
        .color,
    ).toBe("blue");
    expect(
      formatSlackNotification(wrap({ type: "file_deleted", channel: "c" }))
        .color,
    ).toBe("red");
  });
  it("unknown event", () => {
    const n = formatSlackNotification(
      wrap({ type: "other_event", user: "u", channel: "c" }),
    );
    expect(n.color).toBe("gray");
  });
...
describe("mrkdwn conversions", () => {
  ...
  it("html → mrkdwn", () => {
    ...
    expect(typeof convertHtmlToMrkdwn('<a href="https://x">link</a>')).toBe(
      "string",
    );

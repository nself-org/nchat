...
describe("slack/oauth", () => {
  ...
  describe("buildSlackAuthUrl", () => {
    ...
    it("passes team and state through", () => {
      const url = new URL(
        buildSlackAuthUrl(config, { state: "n", team: "T1" }),
      );
  ...
  describe("exchangeCodeForToken", () => {
    ...
    it("throws when ok=false", async () => {
      ...
      await expect(exchangeCodeForToken(config, "c")).rejects.toBeInstanceOf(
        SlackOAuthException,
      );
    });
    ...
    it("throws when ok=true but access_token missing", async () => {
      ...
      await expect(exchangeCodeForToken(config, "c")).rejects.toThrow(
        SlackOAuthException,
      );
    });
  });
  ...
  describe("handleSlackOAuthCallback", () => {
    ...
    it("propagates callback error", async () => {
      await expect(
        handleSlackOAuthCallback(config, { error: "access_denied" }),
      ).rejects.toThrow(SlackOAuthException);
    });

    it("rejects missing code/state", async () => {
      await expect(
        handleSlackOAuthCallback(config, { state: "s" }),
      ).rejects.toThrow(/code/i);
      await expect(
        handleSlackOAuthCallback(config, { code: "c" }),

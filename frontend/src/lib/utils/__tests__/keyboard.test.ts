...
describe("getKeyCombo", () => {
  ...
  it("uses standard symbols when asked", () => {
    const combo = getKeyCombo(kev({ key: "k", shiftKey: true }), {
      useSymbols: true,
    });
...
describe("matchesShortcut", () => {
  it("matches Ctrl+K", () => {
    expect(matchesShortcut(kev({ key: "k", ctrlKey: true }), "Ctrl+K")).toBe(
      true,
    );
  });
  ...
  it("rejects extra modifier", () => {
    expect(
      matchesShortcut(
        kev({ key: "k", ctrlKey: true, shiftKey: true }),
        "Ctrl+K",
      ),
    ).toBe(false);
  });
  ...
  it("accepts object shortcut", () => {
    expect(
      matchesShortcut(kev({ key: "k", ctrlKey: true }), {
        key: "k",
        modifiers: ["ctrl"],
      }),
    ).toBe(true);
  ...
  it("matches by code fallback", () => {
    expect(
      matchesShortcut(kev({ key: "k", code: "KeyK", ctrlKey: true }), "Ctrl+K"),
    ).toBe(true);
  });
});

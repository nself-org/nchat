...
describe("mapValues / mapKeys", () => {
  it("maps values", () => {
    expect(mapValues({ a: 1, b: 2 } as any, (v: any) => v * 2)).toEqual({
      a: 2,
      b: 4,
    });
  });
  ...
  it("maps keys", () => {
    expect(
      mapKeys({ a: 1 } as any, (k) => (k as string).toUpperCase()),
    ).toEqual({ A: 1 });
  });
...
describe("filterObject / compact", () => {
  it("filters by predicate", () => {
    expect(
      filterObject({ a: 1, b: 2, c: 3 } as any, (v: any) => v > 1),
    ).toEqual({ b: 2, c: 3 });
  });

  it("compact removes null/undefined", () => {
    expect(compact({ a: 1, b: null, c: undefined, d: "x" } as any)).toEqual({
      a: 1,
      d: "x",
    });
  });

...
describe("parseUrl", () => {
  ...
  it("supports base URL", () => {
    expect(parseUrl("/path", "https://example.com")?.href).toContain(
      "example.com",
    );
  });
});
...
describe("getQueryParams / getQueryParam", () => {
  it("parses search string", () => {
    expect(getQueryParams("https://a.com?foo=bar&baz=qux")).toEqual({
      foo: "bar",
      baz: "qux",
    });
  });
...
describe("isExternalUrl", () => {
  ...
  it("same origin is not external", () => {
    expect(isExternalUrl("https://example.com", "https://example.com")).toBe(
      false,
    );
  });
...
describe("getUrlDomain", () => {
  ...
  it("handles .co.uk", () => {
    expect(getUrlDomain("https://www.example.co.uk", false)).toBe(
      "example.co.uk",
    );
  });
...
describe("isSameUrl", () => {
  ...
  it("query order doesn't matter", () => {
    expect(isSameUrl("https://a.com?a=1&b=2", "https://a.com?b=2&a=1")).toBe(
      true,
    );
  });
...
describe("addUtmParams", () => {
  it("adds provided utm params", () => {
    const u = addUtmParams("https://a.com", {
      source: "newsletter",
      medium: "email",
    });

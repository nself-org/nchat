...
describe("validatePassword", () => {
  ...
  it("enforces special character requirement when set", () => {
    expect(
      validatePassword("NoSpecial123", { requireSpecial: true }).error,
    ).toMatch(/special/);
    expect(
      validatePassword("Has@Special1", { requireSpecial: true }).valid,
    ).toBe(true);
  });
...
describe("calculatePasswordStrength", () => {
  ...
  it("rates long diverse passwords as strong", () => {
    expect(calculatePasswordStrength("S7r0ng!Pa$$w0rd#Long@2024")).toBe(
      "strong",
    );
  });
...
describe("validateMessageLength", () => {
  ...
  it("custom maxLength works", () => {
    expect(validateMessageLength("too long", { maxLength: 3 }).valid).toBe(
      false,
    );
  });
});
...
describe("validateFileType", () => {
  it("allows any when no config", () => {
    expect(
      validateFileType({ name: "f.xyz", type: "application/foo" }).valid,
    ).toBe(true);
  });
  ...
  it("accepts matching extensions", () => {
    expect(
      validateFileType({ name: "x.pdf" }, { extensions: [".pdf"] }).valid,
    ).toBe(true);
  });
...
describe("validateUrl", () => {
  ...
  it("enforces HTTPS", () => {
    expect(validateUrl("http://a.com", { requireHttps: true }).valid).toBe(
      false,
    );
    expect(validateUrl("https://a.com", { requireHttps: true }).valid).toBe(
      true,
    );
  });
  ...
  it("enforces allowed protocols", () => {
    expect(
      validateUrl("ftp://a.com", { allowedProtocols: ["https"] }).valid,
    ).toBe(false);
    expect(
      validateUrl("https://a.com", { allowedProtocols: ["https"] }).valid,
    ).toBe(true);
  });

  it("enforces allowed domains", () => {
    expect(
      validateUrl("https://bad.com", { allowedDomains: ["good.com"] }).valid,
    ).toBe(false);
    expect(
      validateUrl("https://good.com", { allowedDomains: ["good.com"] }).valid,
    ).toBe(true);
    expect(
      validateUrl("https://sub.good.com", { allowedDomains: ["good.com"] })
        .valid,
    ).toBe(true);
  });
});
...
describe("combineValidations", () => {
  ...
  it("combines warnings", () => {
    const r = combineValidations(
      { valid: true, warning: "w1" },
      { valid: true, warning: "w2" },
    );
    expect(r.valid).toBe(true);

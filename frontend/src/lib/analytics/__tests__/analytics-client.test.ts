...
describe("Analytics Client", () => {
  ...
  describe("identify", () => {
    ...
    it("should filter sensitive traits", () => {
      const client = new AnalyticsClient(
        createTestConfig({ usePrivacyFilter: true }),
      );
      client.initialize(createConsentState(true));
      client.identify("user-123", {
        displayName: "John Doe",
  ...
  describe("track", () => {
    ...
    it("should include base properties", () => {
      const client = new AnalyticsClient(
        createTestConfig({ appVersion: "1.2.3" }),
      );
      client.initialize(createConsentState(true));
      client.identify("user-123");
    ...
    it("should auto-flush when batch size reached", async () => {
      ...
      const client = new AnalyticsClient(
        createTestConfig({ batchSize: 2, onFlush }),
      );
      client.initialize(createConsentState(true));
    ...
    it("should enforce max queue size", () => {
      ...
      for (let i = 0; i < 5; i++) {
        client.track(AnalyticsEvent.PAGE_VIEW, {
          path: `/${i}`,
          title: `${i}`,
        });
      }
  ...
  describe("page", () => {
    ...
    it("should include additional properties", () => {
      ...
      expect((event?.properties as Record<string, unknown>).custom).toBe(
        "value",
      );
    });
  });
  ...
  describe("flush", () => {
    it("should flush events", async () => {
      ...
      expect(onFlush).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({ name: AnalyticsEvent.PAGE_VIEW }),
        ]),
      );
    });
    ...
    it("should restore queue on flush failure", async () => {
      ...
      const client = new AnalyticsClient(
        createTestConfig({ onFlush, onError }),
      );
      client.initialize(createConsentState(true));
  ...
  describe("convenience functions", () => {
    ...
    describe("trackEvent", () => {
      it("should track event using singleton", () => {
        const event = trackEvent(AnalyticsEvent.PAGE_VIEW, {
          path: "/chat",
          title: "Chat",
        });
        expect(event).not.toBeNull();
  ...
  describe("queue persistence", () => {
    it("should persist queue to localStorage", () => {
      const client = new AnalyticsClient(
        createTestConfig({ persistQueue: true }),
      );
      client.initialize(createConsentState(true));
      client.track(AnalyticsEvent.PAGE_VIEW, { path: "/chat", title: "Chat" });
    ...
    it("should not persist when disabled", () => {
      ...
      const client = new AnalyticsClient(
        createTestConfig({ persistQueue: false }),
      );
      client.initialize(createConsentState(true));
      client.track(AnalyticsEvent.PAGE_VIEW, { path: "/chat", title: "Chat" });
      ...
      const queueCalls = calls.filter(([key]: [string]) =>
        key.includes("queue"),
      );

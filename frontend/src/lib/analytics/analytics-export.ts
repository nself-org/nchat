  ...
  try {
    ...
    if (options.sections.includes("summary")) {
      ...
      const summaryData = [
        ...
        [
          "Total Channels",
          data.summary.channels.totalChannels.value.toLocaleString(),
        ],
        [
          "Active Channels",
          data.summary.channels.activeChannels.value.toLocaleString(),
        ],
        [
          "Total Reactions",
          data.summary.reactions.totalReactions.value.toLocaleString(),
        ],
        ["Total Files", data.summary.files.totalFiles.value.toLocaleString()],
        [
          "Total Searches",
          data.summary.search.totalSearches.value.toLocaleString(),
        ],
      ];
    ...
    // Messages section
    if (
      options.sections.includes("messages") &&
    ...
    if (options.sections.includes("users") && data.userActivity.length > 0) {
      ...
      const tableData = formatted.data
        .slice(0, 20)
        .map((row) => [
      ...
      addTable(
        ["User", "Messages", "Reactions", "Engagement Score"],
        tableData,
      );
    }
    ...
    // Channels section
    if (
      options.sections.includes("channels") &&
...
async function exportToXLSX(
  ...
  try {
    ...
    // Summary sheet
    if (options.sections.includes("summary")) {
      const summaryFormatted = formatSummaryForExport(
        data.summary,
        data.dateRange,
      );

...
describe("useRealtime", () => {
  ...
  describe("event handling", () => {
    it("should provide on function", () => {
      ...
      expect(mockRealtimeClient.on).toHaveBeenCalledWith(
        "test-event",
        callback,
      );
    });
    ...
    it("should provide emit function", () => {
      ...
      expect(mockRealtimeClient.emit).toHaveBeenCalledWith("test-event", {
        data: "test",
      });
    });
    ...
    it("should provide emitAsync function", async () => {
      ...
      await act(async () => {
        response = await result.current.emitAsync("test-event", {
          data: "test",
        });
      });

      expect(mockRealtimeClient.emitAsync).toHaveBeenCalledWith("test-event", {
        data: "test",
      });
  ...
  describe("configuration", () => {
    it("should pass debug option to initialize", async () => {
      const { result } = renderHook(() =>
        useRealtime({ autoConnect: false, debug: true }),
      );

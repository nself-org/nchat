...
describe("useRealtimeTyping", () => {
  ...
  describe("initialization", () => {
    it("should return initial state", () => {
      const { result } = renderHook(() =>
        useRealtimeTyping({ roomName: "channel-1" }),
      );
    ...
    it("should set current user ID on mount", () => {
      ...
      expect(mockTypingService.setCurrentUserId).toHaveBeenCalledWith(
        "test-user",
      );
    });
  });
  ...
  describe("typing management", () => {
    it("should start typing in channel", () => {
      ...
      expect(mockTypingService.startTyping).toHaveBeenCalledWith(
        "channel-1",
        undefined,
      );
    });
    ...
    it("should start typing in thread", () => {
      ...
      expect(mockTypingService.startTypingInThread).toHaveBeenCalledWith(
        "channel-1",
        "thread-1",
      );
    });
    ...
    it("should start typing in DM", () => {
      ...
      expect(mockTypingService.startTypingInDM).toHaveBeenCalledWith(
        "dm-1",
        "user-2",
      );
    });
    ...
    it("should stop typing", () => {
      ...
      expect(mockTypingService.stopTyping).toHaveBeenCalledWith(
        "channel-1",
        undefined,
      );
    });
    ...
    it("should stop typing in DM", () => {
      const { result } = renderHook(() =>
        useRealtimeTyping({ roomName: "dm-1", roomType: "dm" }),
      );
...
describe("useThreadTyping", () => {
  ...
  it("should use thread room type", () => {
    const { result } = renderHook(() =>
      useThreadTyping("channel-1", "thread-1"),
    );

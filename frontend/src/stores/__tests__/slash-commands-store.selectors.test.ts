...
function makeState(
  overrides?: Partial<SlashCommandsState>,
): SlashCommandsStore {
...
describe("selectBuiltInCommands", () => {
  ...
  it("returns the builtInCommands array", () => {
    const builtInCommands = [
      makeCommand({ id: "b1" }),
      makeCommand({ id: "b2", trigger: "/b2" }),
    ];
    expect(selectBuiltInCommands(makeState({ builtInCommands }))).toBe(
      builtInCommands,
    );
  });
});
...
describe("selectCustomCommands", () => {
  ...
  it("returns the customCommands array", () => {
    ...
    expect(selectCustomCommands(makeState({ customCommands }))).toBe(
      customCommands,
    );
  });
});
...
describe("selectCommandById", () => {
  ...
  it("returns undefined for a non-existent id", () => {
    ...
    const commands = new Map([["cmd1", cmd]]);
    expect(
      selectCommandById("missing")(makeState({ commands })),
    ).toBeUndefined();
  });
});
...
describe("selectCommandByTrigger", () => {
  ...
  it("returns the command when trigger matches (lowercase)", () => {
    ...
    expect(
      selectCommandByTrigger("/hello")(makeState({ commandsByTrigger })),
    ).toBe(cmd);
  });
  ...
  it("returns undefined for non-matching trigger", () => {
    ...
    const commandsByTrigger = new Map([["/hello", cmd]]);
    expect(
      selectCommandByTrigger("/bye")(makeState({ commandsByTrigger })),
    ).toBeUndefined();
  });
});
...
describe("selectFilteredCommands", () => {
  ...
  it("filters by category when selectedCategory is not all", () => {
    ...
    const media = makeCommand({
      id: "m",
      trigger: "/m",
      category: "media" as never,
    });
    const commands = new Map([
      ["g", general],
      ["m", media],
...
describe("selectIsEditing", () => {
  ...
  it("returns true when editingCommand is set", () => {
    expect(
      selectIsEditing(
        makeState({ editingCommand: { trigger: "/test" } as never }),
      ),
    ).toBe(true);
  });
});
...
describe("selectEditingCommand", () => {
  ...
  it("returns the editingCommand when set", () => {
    ...
    expect(selectEditingCommand(makeState({ editingCommand }))).toBe(
      editingCommand,
    );
  });
});
...
describe("selectRecentExecutions", () => {
  ...
  it("returns up to 10 items by default", () => {
    ...
    const result = selectRecentExecutions()(
      makeState({ executionHistory: history }),
    );
  ...
  it("returns up to the specified count", () => {
    ...
    const result = selectRecentExecutions(5)(
      makeState({ executionHistory: history }),
    );
  ...
  it("returns the first N items in order", () => {
    ...
    const result = selectRecentExecutions(2)(
      makeState({ executionHistory: history }),
    );

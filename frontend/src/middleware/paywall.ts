...
export async function extractPaywallContext(
  ...
  const subscriptionStatus =
    request.headers.get("x-subscription-status") ?? undefined;
...
async function checkFeaturePaywall(
  ...
  return {
    ...
    reason:
      config.errorMessage ??
      `${config.name} is not available on your current plan`,
    currentPlan: context.planTier,
    requiredPlan,
...
async function checkLimitPaywall(
  ...
  if (config.hardLimit && usage >= limit) {
    const requiredPlan = findMinimumTierForHigherLimit(
      context.planTier,
      config.limit,
    );
...
async function checkChannelPaywall(
  ...
  if (
    config.premiumChannelIds &&
    channelId &&
    config.premiumChannelIds.includes(channelId)
  ) {
    // Check if user has required feature
    if (config.requiredFeature) {
      const hasFeature = PLAN_FEATURES[context.planTier][
  ...
  if (
    config.premiumChannelTypes &&
    channelType &&
    config.premiumChannelTypes.includes(channelType)
  ) {
    if (config.requiredFeature) {
      const hasFeature = PLAN_FEATURES[context.planTier][
        config.requiredFeature
...
async function checkTimePaywall(
  ...
  if (config.requireActiveSubscription) {
    ...
    if (
      ...
      !validStatuses.includes(context.subscriptionStatus)
    ) {
      return {
        allowed: false,
        type: "time",
 ...
 */
export function matchRouteToPaywall(
  pathname: string,
...
export function withPaywall(
  ...
  return async function paywallHandler(
    request: NextRequest,
...
function findMinimumTierForHigherLimit(
  ...
  for (let i = currentIndex + 1; i < tiers.length; i++) {
    ...
    if (
      tierLimit === null ||
      (currentLimit !== null && tierLimit > currentLimit)
    ) {
      return tier;
    }
  }
...
export function buildPaywallResponse(result: PaywallCheckResult): NextResponse {
  const response: PaywallErrorResponse = {
    error:
      result.code === PaywallDenialCode.LIMIT_EXCEEDED
        ? "limit_exceeded"
        : "upgrade_required",

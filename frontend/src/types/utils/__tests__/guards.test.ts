...
describe("domain guards", () => {
  ...
  it("channel type narrowing", () => {
    expect(isDirectMessageChannel({ type: "direct", participant: {} })).toBe(
      true,
    );
...
describe("message guards", () => {
  ...
  it("hasAttachments", () => {
    expect(hasAttachments({ ...baseMsg, attachments: [{ id: "a" }] })).toBe(
      true,
    );
  ...
  it("hasReactions", () => {
    expect(hasReactions({ ...baseMsg, reactions: [{ emoji: "x" }] })).toBe(
      true,
    );
  ...
  it("hasThread", () => {
    ...
    expect(hasThread({ ...baseMsg, threadInfo: { replyCount: 0 } })).toBe(
      false,
    );
...
describe("api error guards", () => {
  ...
  it("hasFieldErrors", () => {
    expect(
      hasFieldErrors({ fieldErrors: [{ field: "a", message: "b" }] }),
    ).toBe(true);
...
describe("notification guards", () => {
  ...
  it("isHighPriorityNotification", () => {
    ...
    expect(isHighPriorityNotification({ priority: "urgent" } as any)).toBe(
      true,
    );
    expect(isHighPriorityNotification({ priority: "normal" } as any)).toBe(
      false,
    );
  });
});
